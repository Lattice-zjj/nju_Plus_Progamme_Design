...
{
    ui->setupUi(this);
    this->setMaximumSize(540,338);
    this->setMinimumSize(540,338);
    isWin = false;
    isFailure = false;
    is_begin = false;
    frameWidth = 540;
    ...
    update1();
    //失败动画
    overMovie.setFileName("://res/over.gif");

    winMovie.setFileName("://res/win.gif");

    ui->helper1->setStyleSheet("border-image:url(://res/helper1.JPG);");
    ui->helper2->setStyleSheet("border-image:url(://res/helper2.JPG);");
    ui->helper3->setStyleSheet("border-image:url(://res/helper3.JPG);");

    connect(ui->action_begin,&QAction::triggered,
            [=](){
    ...
    connect(ui->action_helpfile,&QAction::triggered,
            [=](){ QDesktopServices::openUrl (QUrl::fromLocalFile("./HelpFile/HelpFile.pdf"));
    });
    connect(ui->action_end,&QAction::triggered,this,&QMainWindow::close);
...
void MainWindow::paintEvent(QPaintEvent *){
    QPainter p(this);
    p.drawPixmap(rect(),QPixmap("://res/back.JPG"));
...
int MainWindow::randomInt( int high )
{
    qsrand(uint(QTime(0,0,0).secsTo(QTime::currentTime())));
    int k=qrand()%(high+1);
...
QColor MainWindow::getcolor(int x){
...
void MainWindow::moveKnockRight()
{
...
void MainWindow::setBallSpeed( qreal speed )
{
    ball->setSpeed( speed );
}

void MainWindow::setPaddleStep( qreal step )
{
    knock->setStep( step );
}


void MainWindow::on_changeScore( int score )
{
